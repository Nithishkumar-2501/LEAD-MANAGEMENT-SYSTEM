...
export default function Sidebar({
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      if (!target.closest("#nav-category-appmanager") && !target.closest("#flyout-appmanager") && !target.closest("#dropdown-appmanager")) {
        if (isCollapsed) setIsAppManagerOpen(false);
      }
    }
  ...
  const applicationManagerSubItems = [
    {
      id: "APPLICATION_MANAGER" as ActiveTab,
      label: "Manage Applications",
      sublabel: "Central Applications Register",
      icon: FileText,
    ...
    {
      id: "APPLICATION_OFFLINE_LOGS" as ActiveTab,
      label: "Offline Application upload logs",
      sublabel: "Excel & CSV Batch Ingestion",
  ...
  const isAppManagerActive = activeTab === "APPLICATION_MANAGER" || activeTab === "APPLICATION_OFFLINE_LOGS";
  const isAdmissionActive = filteredSubItems.some((item) => item.id === activeTab) || (!isDashboardActive && !isSocialActive && !isAppManagerActive);
  ...
  const [isAppManagerOpen, setIsAppManagerOpen] = useState(isAppManagerActive);
  ...
  useEffect(() => {
    if (dashboardSubItems.some((item) => item.id === activeTab)) {
      ...
      setIsAdmissionCrmOpen(false);
      setIsAppManagerOpen(false);
    } else if (activeTab === "APPLICATION_MANAGER" || activeTab === "APPLICATION_OFFLINE_LOGS") {
  ...
  const toggleAppManagerMenu = () => {
    setIsAppManagerOpen((prev) => {
      const next = !prev;
      if (next) {
        setIsDashboardOpen(false);
        setIsAdmissionCrmOpen(false);
        setIsSocialPlatformOpen(false);
        if (activeTab !== "APPLICATION_MANAGER" && activeTab !== "APPLICATION_OFFLINE_LOGS") {
          onTabChange("APPLICATION_MANAGER");
        }
      }
      return next;
    });
            ...
            <div className="relative space-y-1.5 pt-2 border-t border-slate-200 dark:border-white/10 mt-2">
              {isCollapsed ? (
                <Tooltip text="Application Manager" position="right">
                  <button
                    onClick={toggleAppManagerMenu}
                    className={`w-full flex items-center justify-center p-3 rounded-2xl transition-all duration-300 relative press-spring ${
                      isAppManagerActive
                        ? "bg-gradient-to-r from-sky-500 to-indigo-600 text-white shadow-xl shadow-sky-500/30 border border-sky-400 scale-105"
                        : isLight
                        ? "bg-slate-100 text-slate-800 hover:bg-slate-200 border border-slate-200"
                        : "bg-slate-900/80 text-slate-300 hover:text-white border border-white/10 hover:border-white/20"
                    }`}
                ...
                <div
                  id="nav-category-appmanager"
                  className={`w-full flex items-center justify-between px-3.5 py-3 rounded-2xl text-xs font-black transition-all duration-300 cursor-pointer group border card-hover-elevate press-spring shadow-xs ${
                    isAppManagerOpen || isAppManagerActive
                      ? isLight
                        ? "bg-sky-50/90 border-sky-300 text-slate-900 shadow-md ring-1 ring-sky-300/40"
                        : "bg-sky-950/60 border-sky-500/50 text-sky-200 shadow-lg shadow-sky-950/40 ring-1 ring-sky-500/30"
                      : isLight
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div
                      className={`w-10 h-10 rounded-2xl shrink-0 flex items-center justify-center transition-all duration-300 shadow-xs ${
                        isAppManagerOpen || isAppManagerActive
                          ? "bg-sky-200 dark:bg-sky-900/80 text-sky-800 dark:text-sky-200 shadow-sm ring-2 ring-sky-300/40"
                          : isLight
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <div className="p-0.5 rounded-lg transition-transform">
                      <ChevronRight
                        className={`w-4 h-4 transition-transform duration-300 ${
                          isAppManagerOpen ? "rotate-90 text-sky-500" : "text-slate-400 group-hover:translate-x-0.5"
                        }`}
                      />
                    </div>
                  </div>
              ...
              {/* EXPANDED STATE INLINE ACCORDION DROPDOWN FOR APPLICATION MANAGER */}
              {isAppManagerOpen && !isCollapsed && (
                <div
                  id="dropdown-appmanager"
                  className="space-y-1.5 pt-1.5 animate-in fade-in slide-in-from-top-1 duration-200"
                >
                  {applicationManagerSubItems.map((item) => {
                    const Icon = item.icon;
                    const isActive = activeTab === item.id;
                    return (
                      <button
                        ...
                        id={`nav-${item.id.toLowerCase().replace(/_/g, "-")}`}
                        onClick={() => handleNavClick(item.id)}
                        className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-all duration-200 group relative cursor-pointer ${
                          isActive
                            ? `bg-gradient-to-r ${item.color} text-white shadow-lg ${item.activeGlow} border ${item.activeBorder} scale-[1.02]`
                            : isLight
                            ? "bg-slate-100/70 hover:bg-slate-200/80 text-slate-700 hover:text-slate-900 border border-slate-200"
                            : "bg-slate-900/60 hover:bg-slate-800 text-slate-300 hover:text-white border border-white/5 hover:border-white/15"
                        }`}
                      >
                        <div
              ...
              {/* COLLAPSED STATE RIGHT SIDE FLYOUT POPOVER FOR APPLICATION MANAGER */}
              {isCollapsed && isAppManagerOpen && (
                <div
                  id="flyout-appmanager"
                  className={`fixed left-20 top-60 z-50 w-64 p-2.5 rounded-2xl border shadow-2xl space-y-1.5 backdrop-blur-2xl animate-in fade-in slide-in-from-left-2 duration-150 ${
                    isLight
                      ? "bg-white/95 text-slate-900 border-slate-200 shadow-slate-300/50"
                      : "bg-slate-950/95 text-white border-white/15 shadow-black/80"
                  }`}
                >
                  <div className="px-2 py-1 border-b border-white/10 flex items-center justify-between mb-1">
                    <span className="text-[10px] font-black uppercase tracking-wider text-sky-400 flex items-center gap-1">
                      <FileCheck className="w-3 h-3" /> Application Manager
                    </span>
                    <span className="text-[9px] font-extrabold px-1.5 py-0.5 rounded bg-sky-500/20 text-sky-300">
                      Admin Only
                    </span>
